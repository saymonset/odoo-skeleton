...
class WhatsappComposeMessage(models.TransientModel):
    ...
    def whatsapp_message_post_new(self, server, KlikApi):
        try:
                        ...
                        if partners:
                            for partner in partners:
                                ...
                                if partner.whatsapp and partner.whatsapp != '0' and whatsapp not in opt_out_list:                          
                                    ...
                                    message_data = {
                                        'method': 'sendMessage',
                                        'client_secret': server.klik_secret,
                                        'phone': whatsapp,
                                    ...
                                    if not attachment_new_ids and message_data['body']:
                                        ...
                                        send_message = KlikApi.post_request(token=server.whatsapp_token, data=payload)
                                        ...
                                        chatID = partner.chat_id if partner.chat_id else whatsapp
    ...
    def whatsapp_message_post(self):
        ...
        try:
            for wserver in whatsapp_ids.filtered(lambda ws: not ast.literal_eval(str(ws.message_response))['block']):
                ...
                KlikApi = wserver.klikapi()
                # KlikApi.auth()
                thread_start = threading.Thread(target=self.whatsapp_message_post_new(wserver, KlikApi))
                thread_start.start()
