...
class FacturaglobalLine(models.Model):
    ...
    def _prepare_base_line_for_taxes_computation(self, **kwargs):
        ...
        return self.env['account.tax']._prepare_base_line_for_taxes_computation(
            self,
            **{
                'tax_ids': self.invoice_line_tax_ids,
                'quantity': self.quantity,
                'partner_id': self.factura_global_id.partner_id,
                'currency_id': self.factura_global_id.currency_id or self.factura_global_id.company_id.currency_id,
                'rate': 1, #self.factura_global_id.currency_rate,
                **kwargs,
            },
        )
...
class Facturaglobal(models.Model):
    ...
    def _compute_tax_totals(self):
        ...
        for order in self:
            #order_lines = order.factura_line_ids.filtered(lambda x: not x.display_type)
            base_lines = [line._prepare_base_line_for_taxes_computation() for line in order.factura_line_ids]
