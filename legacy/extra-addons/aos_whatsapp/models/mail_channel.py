...
class mailChannel(models.Model):
    ...
    def message_post(self, *, message_type='whatsapp', **kwargs):
        ...
        if not self._context.get('from_odoobot') and self._context.get('uid'):
            self.send_whatsapp_message(message, kwargs)
            #self.send_whatsapp_message(self.message_ids, message, kwargs)
        return message
    ...
    def send_whatsapp_message(self, message_id, kwargs):
        print ('==send_whatsapp_message=', self, message_id, kwargs)
        partner_id = False
        ...
        whatsapp_endpoint = self.env['ir.config_parameter'].sudo().get_param('aos_whatsapp.url_klikodoo_whatsapp_server') or 'https://klikodoo.id/api/wa'
        ...
        if self.whatsapp_number:#partner_ids and message_id.author_id and not partner_id:
            ...
            if whatsapp_endpoint and whatsapp_token:
                ...
                html_to_plain_text = html2plaintext(kwargs.get('body'))
                print ('===html_to_plain_text==',whatsapp_endpoint,whatsapp_id,html_to_plain_text)
